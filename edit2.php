...
<html>
<head>
    <title>EDIT LAPORAN SURAT KELUAR</title>
...
<?php 
// Mengecek apakah parameter GET id_srt_k ada pada URL
if (isset($_GET['id'])) {
    // Menyimpan nilai id_srt dari parameter GET ke variabel $id_srt
    $id = $_GET['id'];

    // Mengambil data dari tabel tambahsm berdasarkan id_srt
    $select = mysqli_query($koneksi, "SELECT * FROM tambahsk WHERE id_srt_k='$id'") or die(mysqli_error($koneksi));
...
if (isset($_POST['submit'])) {
    ...
    $sql = mysqli_query($koneksi, "UPDATE tambahsk SET id_srt_k='$id_srt_k', no_srt='$no_srt', tgl_srt='$tgl_srt', tujuan='$tujuan', hal='$hal', ket='$ket' WHERE id_srt_k='$id_srt_k'") or die(mysqli_error($koneksi));
...
?>
    ...
    <form action="edit2.php?id=<?php echo $id; ?>" method="post">
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<input type="text" name="id_srt_k" class="form-control" value="<?php echo $data['id_srt_k']; ?>" required>
				</div>
			</div>
			<div class="form-group row">
